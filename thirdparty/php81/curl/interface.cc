...
static curl_result_t php_curl_option_str(php_curl *ch, zend_long option, const char *str, const size_t len) {
    {
...
#if PHP_VERSION_ID >= 80300
static curl_result_t php_curl_option_url(php_curl *ch, const zend_string *url) /* {{{ */
#else
static curl_result_t php_curl_option_url(php_curl *ch, const char *url, const size_t len) /* {{{ */
#endif
{
    ...
    if (len > sizeof("file://") - 1 && '/' != url[sizeof("file://") - 1] &&
        !strncmp("file://", url, sizeof("file://") - 1) && len < MAXPATHLEN - 2) {
        char _tmp[MAXPATHLEN] = {0};
...
static inline curl_result_t build_mime_structure_from_hash(php_curl *ch, zval *zpostfields);
SW_EXTERN_C_END
...
static zend_object *swoole_curl_clone_obj(zend_object *object) {
    ...
    clone_object = swoole_curl_create_object(swoole_coroutine_curl_handle_ce);
...
static int fn_debug(CURL *cp, curl_infotype type, char *buf, size_t buf_len, void *ctx) /* {{{ */
{
    php_curl *ch = (php_curl *) ctx;
...
#else
static inline CURLcode add_simple_field(struct HttpPost **first,
...
static curl_result_t _php_curl_setopt(php_curl *ch, zend_long option, zval *zvalue, bool is_array_config) /* {{{ */
{
    CURLcode error = CURLE_OK;
    zend_long lval;
...
#endif
        curl_result_t ret = php_curl_option_str(ch, option, ZSTR_VAL(str), ZSTR_LEN(str));
...
#if PHP_VERSION_ID >= 80300
        curl_result_t ret = php_curl_option_url(ch, str);
#else
        curl_result_t ret = php_curl_option_url(ch, ZSTR_VAL(str), ZSTR_LEN(str));
#endif
...
#if LIBCURL_VERSION_NUM >= 0x075400 && PHP_VERSION_ID >= 80300 /* Available since 7.84.0 */
...
#if LIBCURL_VERSION_NUM >= 0x074800 && PHP_VERSION_ID >= 80200 /* Available since 7.72.0 */
...
#else
    swoole_curl_verify_handlers(ch, /* reporterror */ 0);
#endif
